...
namespace DigitalDetector.infra.service
{
    public class Usuario_Service
    {
        ...
        public Usuario CarregarUsuario(string nome, string base64)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(conn))
                {
                    SqlDataAdapter cmd = new SqlDataAdapter("SELECT Id, Nome,Digital from Usuarios where Nome = @nome and Digital = @digital", con);
        ...
        public bool CriarUsuario(Usuario usuario)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(conn))
                {
                    SqlCommand cmd = new SqlCommand("Insert into Usuarios(Id,Nome,Digital) values (@id,@nome,@digital)", con);
